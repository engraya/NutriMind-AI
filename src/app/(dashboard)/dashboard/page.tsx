...
async function getDashboardData(userId: string) {
  ...
  const [profileRes, todayLogsRes, waterLogsRes, recentMealsRes, weeklyLogsRes] =
    await Promise.all([
...
export default async function DashboardPage() {
  ...
  const { profile, today, water, recentMeals, weeklyData } =
  ...
  return (
    <div className="space-y-5">
      ...
      {/* KPI Cards */}
      <DashboardGrid className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <DashboardGridItem>
        ...
        <DashboardGridItem>
          <WaterIntakeCard
            totalMl={water}
            goalMl={
              profile?.weight_kg ? Math.round(profile.weight_kg * 35) : 2500
            }
            userId={user.id}
          />
        </DashboardGridItem>
